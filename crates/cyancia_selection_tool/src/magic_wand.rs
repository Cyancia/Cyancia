...
use cyancia_image::tile::{GpuTileInfo, GpuTileStorage};
...
impl ToolFunction for MagicWandSelectionTool {
    ...
    fn begin(&mut self, mouse: &MouseDownEvent, cx: &mut Context<Self>) {
        ...
        unsafe { render_context.device.start_graphics_debugger_capture() };
        let Some((output_texture, output_tiles, output_tile_info_buffer)) = bucket.dispatch_mask(
            &render_context.device,
        ...
        unsafe { render_context.device.stop_graphics_debugger_capture() };
        ...
        selection_pipeline.composite(
            ...
            &output_texture,
            &output_tiles.iter().copied().collect(),
        ...
        let selection_layer = tiles.get_layer(canvas.image.selection_layer()).unwrap();
