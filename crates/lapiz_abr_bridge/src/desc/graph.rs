...
pub fn computed_graphs(
    ...
    let required_spacing_graph = computed_required_spacing_graph(diameter, spacing)?;
    let main_graph = computed_main_graph(diameter, hardness, angle, roundness, flip_x, flip_y)?;
    Ok((required_spacing_graph, main_graph))
}

fn computed_required_spacing_graph(diameter: f32, spacing: f32) -> Result<SerializableGraph> {
